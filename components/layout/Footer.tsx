...
const Footer = (): JSX.Element => {
  return (
    <footer className='bg-lightBlue flex flex-col gap-8 items-center justify-center pb-8'>
      {/* SQUARED LOGO */}
      <div className='relative flex justify-center w-full h-fit'>
        <div className='absolute top-0 left-0 bg-white h-[50%] w-full'></div>
        <div className='flex justify-center items-center relative bg-gray-900 text-white aspect-square w-28'>
          <svg
      ...
      <nav className='flex flex-col gap-8 items-center'>
        {data.nav.map(([href, label]) => (
          <Link href={href} passHref key={`navLink-${href}`}>
            <a className='relative items-center capitalize hover:text-black transition-colors'>
              {label}
            </a>
          </Link>
        ))}
